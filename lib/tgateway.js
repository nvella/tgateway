...
TGateway.prototype.run = function() {
  ...
  for(var file of fs.readdirSync('templates')) {
    ...
    // Trim off a new line if it exists
    if(template.charCodeAt(template.length - 1) === 10) template = template.substring(0, template.length - 1);
  ...
  for(var id in this.config.backends) {
    ...
    if(this.backendDrivers[obj.type] === undefined) {
      ...
      // Initialize the backend with the config
      var backend = new (this.backendDrivers[obj.type])(this, obj);
...
TGateway.prototype.connectFrontEndpoint = function(endpoint, frontend) {
  ...
  this.connections.push(connection); // Push the connection onto the connections array
}
